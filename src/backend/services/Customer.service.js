 ...
 * @param {string} rank - The rank to filter by
 * @returns {Promise<Array>} A promise containing an array of customers with the given rank
 */
exports.getCustomersByRank = async (rank) => {  
  return await Customer.findAll({
    where: {
      rank: {
        [Op.regexp]: rank // Searches using regular expression
      }
    },
    attributes: { exclude: ["createdAt", "updatedAt", "refresh_token"] },
    order: [["createdAt", "DESC"]]
